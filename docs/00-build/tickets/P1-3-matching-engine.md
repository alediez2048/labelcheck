...
P1-3 is the correctness core. Every decision in this engine is **code** — D4 and D5 say the model only reads and the code only decides, and this is where that promise is operationalised. The biggest design choice was separating the per-field matchers (one file each: `abv.ts`, `netContents.ts`, `origin.ts`, `fuzzy.ts`, `warning.ts`) from the orchestrator (`match.ts`), with `MatchResult` as the shared output type. The alternative — one big switch statement — would have looked tidier but conflated two responsibilities: dispatching the right rule (orchestrator) and applying the rule (per-field). When P5-2 starts calibrating thresholds against the golden set, the calibration work will touch a single file per field; the orchestrator stays inert.

`MatchResult` carries **`margin`** instead of `confidence` for a specific reason: P1-4 derives confidence in code from `margin` plus the model's per-region legibility flag (D5). If we'd put `confidence` here, P1-4 would have to either accept the matcher's number or override it, and override-mode is the kind of seam that silently breaks when someone forgets why it exists. By making `margin` the contract, P1-4's role is unambiguous — it's the **only** place confidence is ever assigned. The `MatchResult` type lives in its own file (`types.ts` inside `lib/matching/`) rather than in `@/types` because it's an **internal** type — the public domain contract is `FieldResult` (with confidence), produced by P1-4.

The **camelCase ↔ snake_case translation** is back for the third time (P0-2 type contract, P1-2 extraction, here). The form-side uses camelCase because that's the TypeScript convention; the wire-side (`FieldName`, the `field_result` audit identifiers, the matching engine's lookup keys) uses snake_case because that's the schema vocabulary. The map lives in `match.ts` rather than in a shared utility because the boundary **is** the matcher — it's where the form and the label meet. Pulling it into a shared `lib/translate` module would obscure the intent.

**`parseNetContents` was rewritten twice during this ticket.** The first version used `\b` word boundaries which fail between digit and letter (so `"750ml"` with no space didn't match `\bml\b`), and blanket-stripped periods to handle `"fl. oz."` — which also ate the decimal in `"0.75 L"`. Both bugs surfaced only when the tests ran them through; the rewritten version uses negative lookbehind/lookahead (`(?<![a-z])ml(?![a-z])`) and tolerates optional trailing periods explicitly in the unit regex. This is exactly the kind of subtle parsing bug an LLM-as-judge would let through — Levenshtein-on-strings would have called `"750ml"` and `"0.75 L"` similar; the structural parser catches the unit mismatch.

The **warning matcher's evaluation order is deliberate**: presence first, then we look for transcribed text; if `presence: true` but no text, that's `low_confidence` (the human should look). Then ALL CAPS strict (FR-11). Then verbatim byte-for-byte. **Bold last**, because bold is best-effort (D6) — we already passed the strict checks before considering the styling cue. `boldConfident: "uncertain"` downgrades the overall verdict to `low_confidence` rather than fail or pass; the failure mode that matters is an auto-fail on a flaky styling read, and routing to human review is the safe default.

The **legibility flag is consumed at this layer but doesn't change the verdict**; the docstring notes it. P1-5 (triage) is where a `legibility: "low"` flag would lower confidence enough to push a marginal `match` into the `review` lane. We resisted folding that logic into the warning matcher because it would couple two concerns: "does the warning satisfy the rule?" (P1-3) and "is the confidence high enough to clear?" (P1-5). Keeping them separate means P5-2's calibration can tune the thresholds independently.

**Test fixtures pass the warning config as a parameter** rather than relying on `config/warning.json` (which still has the A18 placeholder). This is dependency-injection-over-hidden-globals — production reads from disk, tests pass fixed values. The alternative (mocking `getWarningConfig`) would have worked but is heavier; passing the config keeps the test surface small and the matcher itself purer.
