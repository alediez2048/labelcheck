...
P0-3 is the load-bearing seam: every model integration (today's mock, P1-2's Claude Sonnet 4.6, P6-1's Azure OpenAI or self-hosted olmOCR) sits behind one interface, so the rest of the system never knows or cares which one is on. We made the interface as narrow as it could possibly be — one method `extract()`, one input shape, one output shape — because every field added today is a coupling we have to maintain across every future provider; the production-migration story in P6-1 only works if this stays narrow. The response is per-face **text** plus warning structural flags — no `verdict`, no `match`, no overall confidence number. We considered exposing the model's self-reported confidence "just in case" and rejected it: the matching engine (P1-3) and the triage classifier (P1-5) compute confidence in code (D5), and a model-confidence field on `ExtractionResponse` would silently get consumed by some future hot fix and quietly bring back the exact anti-pattern D5 was written to prevent. `boldConfident` is a three-value flag (`yes | no | uncertain`), not a boolean, because D6 makes bold detection best-effort — a boolean would force a false binary on an unreliable read, and "uncertain" routes the case to the review lane instead of forcing a verdict on a styling cue.

The mock comes first because every later ticket needs a working extraction without an API key — P1-3 matching, P1-5 triage, P1-7 result API, P1-8 review UI, even P5-2 evals can all be built and CI'd against it. The three fixtures (`sample-green-001`, `sample-abv-mismatch-001`, `sample-warning-titlecase-001`) cover the three lanes (match, mismatch, review) plus the hardest field (the government warning), so any consumer can exercise every branch. Crucially, the mock returns the same response shape every real provider must — if the mock is sloppy with optional fields, the live adapter in P1-2 and the in-boundary adapter in P6-1 will silently diverge and the matching engine will break. `Partial<Record<FieldName, string>>` for the per-face fields map mirrors reality: the model often won't find every field on every face, and the multi-face merge (P1-6) reconciles across faces per D12.

`getProvider()` is env-driven (`PROVIDER=mock` default) because that matches every dev / test / CI workflow without code edits, and because D8 says swappable-by-config — we lock that in now even with one impl. We explicitly throw clear errors for known live provider names (`anthropic`, `azure-openai`, `olmocr`) with pointers to the tickets that land each, rather than silently falling back to the mock and pretending to verify against a real model. The trade-off accepted on tests: full Vitest isn't installed until P0-7, so the test file uses **type-level guards** (`_ResponseHasNoVerdict`, `_ResponseHasNoOverallConfidence`) that `pnpm build` catches at compile time, plus runtime `describe`/`it` blocks that Vitest auto-discovers once installed. The skeleton compiles today; the runtime assertions run tomorrow. We considered waiting for P0-7 to write tests at all — rejected because the type-level guards are the single most important enforcement: they fail the build if a future agent adds a `verdict` field to the response, which is the change we most want to prevent. `lib/provider/README.md` exists because a future agent opening this directory should understand the contract before touching it; the alternative — leaving the rationale only in design docs — invites the exact "let me just add one field" drift that breaks the production migration. `zod` is installed but unused by the mock; staged for P1-2's runtime validation of live-provider responses where the type system alone can't catch shape mismatches at the wire boundary.
