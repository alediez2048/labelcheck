...
P1-2 is two related deliverables — the extraction service that converts a validated Application into a model call, and the live Anthropic Claude provider behind the same `VisionProvider` interface the mock has been satisfying since P0-3. We kept them in one ticket because either alone is a half-build: the extraction service with no live provider is just a mock orchestrator; the live provider with no service to call it is a leaf module nobody invokes. Together they close the loop from form submission to model response.

The biggest call was **one call per Application, all faces attached** (D14). The temptation a future agent will face is to "improve parallelism" by calling the model per face and merging the responses — that would silently break two things: the 5-second p95 latency budget (NFR-1) once you add three round trips of overhead, and the per-application cost model (NFR-3 / business.md), which assumes one model call per application. The implementation enforces this structurally — there is one `provider.extract()` invocation; the only loop is the concurrent `Promise.all` over **preprocessing**, which doesn't touch the model.

The **camelCase form-key → snake_case `FieldName` translation** is the kind of seam that, left implicit, eats a day six tickets later. The config uses camelCase because the form-side `FormFields` shape does; the wire vocabulary (`FieldName`, the matching engine's lookup keys, the `field_result` row identifiers in schema.md) is snake_case. The `CONFIG_KEY_TO_FIELD_NAME` map lives in the extraction service rather than in the config schema because the translation is where the form-side meets the wire-side — putting it in `lib/config` would push form-side knowledge into the config loader. The first test failure (`country_of_origin` not in the schema) was the **right** failure mode: the matching engine in P1-3 reads the schema by snake_case names, and a silently-empty schema would mean the model is never asked for half the fields.

The **`ReadonlyArray` change on `ExtractionRequest.faces` and `fieldSchema`** is a small but principled correctness move. Provider implementations have no business mutating the request — that's a code smell that would suggest a provider keeping per-request state somewhere it shouldn't. Making the type read-only enforces the contract at compile time and made the build fail on the first try, which was exactly what we wanted. The mock and Anthropic provider both consume via `.map()` which works on readonly arrays, so the change cost nothing.

The Anthropic adapter uses **base64-in-multimodal-content** rather than uploading via the Files API. We accepted the bytes-in-payload trade-off because (1) the Files API adds a separate upload round trip per face, multiplying the request count we just structurally promised was 1 (D14); (2) the per-application image payload at the 1568px cap is well under the 5MB Anthropic message limit; (3) Files API requires retention reasoning that doesn't fit NFR-4's "no persistence" rule. Base64 inline matches what `lib/provider/mock.ts` already returns and keeps the cost model simple.

The **JSON parsing tolerance** (`parseJsonStrict` accepting code-fence-wrapped output) is the one defensive bit. The prompt explicitly says "no markdown, no code fences" but Sonnet models sometimes ignore that instruction. We chose to be lenient on the wrapper rather than fail-strict because the alternative — failing on every fenced response — would mean failing on a transient model behavior we can't control, when the actual content is fine. P5-1 will track fenced-vs-clean responses as a rolling signal so we'd notice if the wrapper rate climbs. `ANTHROPIC_MODEL` is env-overridable specifically so P5-4's bake-off can swap models without code edits. The cost-per-case estimate of $0.025 is a rough number from techstack's $3,825/year envelope at 150K applications. We resisted adding tool use / structured outputs here — the JSON-prompt path works against the current model and adds zero new SDK surface to test against; a future ticket swaps if the JSON failure rate is non-trivial in P5-2 evals.
