...
- Depends: P0-3, P0-5, P1-1, P2-2, P3-1, P5-3, P5-4 · Branch: feat/dragdrop-intake · Est: 6h
- Goal: replace the in-memory seed queue with an empty initial state; add an admin-facing drag-and-drop intake on Operations that accepts single PDFs, multi-select, or whole folders. **PDF rasterisation + text extraction run in the BROWSER via `pdfjs-dist` in a Web Worker** so the Vercel server never loads a PDF processor. Form fields parsed client-side (regex first, vision-provider fallback for missing fields). Each application submits through the existing `/api/batch` seam carrying a base64 PNG + assembled form. A "Load demo prefill" button stays as a one-click curated 10-green + 3-red set using pre-rendered committed PNGs.
- Acceptance: [ ] `QueueProvider.INITIAL_STATE` boots empty (agents + role defaults preserved); Analytics empty-state CTA in place; [ ] dropzone accepts single PDFs, multi-select, and Chrome/Edge folder uploads (Safari multi-file fallback); non-PDFs rejected per-file; respects `config/batch.json` maxItems; [ ] `components/intake/PdfWorker.ts` owns a single `pdfjs-dist` instance in a Web Worker; renders label page (~1568px long edge) + extracts text for client-side regex parse; [ ] `lib/intake/client/parseColaPdf.ts` ports the Python regex anchors from `tools/cola-fetcher/pull_colas.py`; when required fields are missing, the worker also renders the form page and the payload attaches it as a `kind: "form_page"` face with `formFallbackHint: { missingFields }` so the orchestrator can run a second provider call server-side with a distinct form-extraction prompt; regex wins on conflict; every field tagged `source: regex|vision|operator|missing`; [ ] "Load demo prefill" loads exactly 13 items (10 curated TTB greens from `data/sample-colas/manifest.curated.csv` with form fields baked in + 3 synthetic reds via `SYNTHETIC_FIXTURE_META`); 10 prefill label PNGs committed under `public/sample-colas/` so prefill never invokes the worker; [ ] "Reset demo state" clears in-memory state but NOT `eval-data/agent-corrections/`; [ ] live grading via Anthropic when `ANTHROPIC_API_KEY` is set, mock fallback with banner otherwise; 3 reds always mock-graded (AC-2/AC-3/AC-4 deterministic); [ ] every row + review detail shows its own label image (placeholder bug fixed by construction); [ ] **the server never loads `pdf-parse`, `pdf-to-png-converter`, `pdf-poppler`, or any PDF processor from any module reachable from `/api/batch` (Vercel safety)** — `pdf-parse` stays available only to `lib/kb/**` for P4-1 KB ingestion; [ ] Vercel preview deploy verified: drop → grade → render works end-to-end on the deployed URL; function bundle size logged in DEV-LOG; [ ] `pnpm eval --gate` still passes.
- Refs: PRD Phase 1; NFR-1; NFR-2; NFR-4; AC-2/AC-3/AC-4/AC-5/AC-9; D4/D8/D11/D15; P5-7 deploy plan; tools/cola-fetcher/pull_colas.py; data/sample-colas/README.md.

### P5-6 — Design alignment with the mockup
- Depends: P5-8, P2-6, P1-8, P4-3 · Branch: feat/design-alignment · Est: 5h
- Goal: bring the running app's visuals into exact alignment with `docs/03-ui/mockup.html` — same tokens, type, spacing, components, chrome; behaviour frozen.
- Acceptance: [ ] design tokens from the mockup land in `tailwind.config.ts` as semantic colors and shadow; [ ] sidebar, topbar, panel/card pattern, funnel strip, queue rows, review-detail two-column layout, and assistant FAB+dock all match the mockup within reason; [ ] color+icon+text everywhere preserved (NFR-2, AC-9); [ ] `pnpm eval --gate` still passes — design pass cannot regress matching logic.
- Refs: `docs/03-ui/mockup.{html,md}`; NFR-2; AC-9; FR-13/14/15/21/22/24.

### P5-7 — Deployment plan (reviewable take-home)
- Depends: P0-6, P5-5, P5-6 · Branch: feat/deployment-plan · Est: 3h
- Goal: make the take-home reviewable as a live URL behind the access-gate spend shield, and publish `docs/DEPLOYMENT.md` with the env-var inventory, the deliberate "nothing persisted" choice (NFR-4), and the Phase 6 gap matrix (which ticket closes which prod gap).
- Acceptance: [ ] `docs/DEPLOYMENT.md` published with the env-var inventory + the Phase 6 gap matrix; [ ] `.env.example` committed; [ ] README "Live demo" / "Run locally" section linked to the doc; [ ] (Tier 2) `vercel.json` committed + live URL accessible behind the passcode; [ ] CI's eval-gate documented as the deploy gate (a regression that fails the gate blocks the deploy).
- Refs: PRD § "credible path to production"; NFR-4; NFR-8; assumption A21; Phase 6 tickets P6-1 through P6-6.

Phase 5 exit: eval runs produce the metrics; agent corrections accumulate as an eval set; a change that regresses the golden set fails CI; the running app matches the mockup; the take-home is reviewable via a documented deploy.
