/**
 * GLM-OCR vision adapter (Zhipu AI, China-origin).
 *
 ...
 * candidates on the same surface (D8). Requires `GLM_OCR_ENDPOINT`; throws
 * "GLM-OCR endpoint not configured" if missing.
 */
...
export class GlmOcrVisionProvider implements VisionProvider {
  readonly name = "glm-ocr";
  private readonly endpoint: string;

  constructor() {
    const endpoint = process.env.GLM_OCR_ENDPOINT;
    if (!endpoint) {
      throw new Error(
        "GLM-OCR endpoint not configured (set GLM_OCR_ENDPOINT). PENDING SECURITY REVIEW — not approved for Treasury workloads without executive review.",
      );
    }
  ...
  async extract(input: ExtractionRequest): Promise<ExtractionResponse> {
    ...
    try {
      ...
      throw new Error(
        `GLM-OCR endpoint unreachable at ${this.endpoint}: ${detail}`,
      );
    }

    if (!response.ok) {
      throw new Error(
        `GLM-OCR endpoint returned ${response.status} ${response.statusText}`,
      );
    }
    ...
    if (!validated.success) {
      throw new Error(
        `GLM-OCR response did not match schema: ${validated.error.message}`,
      );
    }
  ...
  rereadWarning(_input: WarningRereadInput): Promise<WarningRereadResponse> {
    return Promise.reject(
      new Error("GLM-OCR re-read not implemented — mock provider only"),
    );
  }
}
